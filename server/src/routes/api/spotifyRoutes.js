...
router.get("/categories", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch categories from Spotify API" });
  }
});
...
router.get("/:subjects", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api.spotify.com/v1/browse/categories/${subjects}/playlists`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch playlists from Spotify API" });
  }
});
